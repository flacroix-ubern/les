...
#| fig-cap: "Albedo values for different vegetation types. Each point represents an annual averate for a site where surface radiation fluxes were measured. Vegetation types are described in @tbl-vegtype. Data from @cescatti12rse."
#| out-width: 60%
...
Different metrics are used for quantifying the partitioning of net radiation into $H$ and $\lambda E$. When considering totals over longer periods of time (over at least one annual cycle), the ground heat flux $G$ can be neglected since the ground doesn't continuously heat up over time. Hence, the sum of the latent and sensible heat fluxes adds up to match net radiation. The *evaporative fraction* quantifies the fraction of available energy from net radiation that goes into the latent heat flux. (The remainder goes into the sensible heat flux.)
$$
\mathrm{EF} = \lambda E/(\lambda E + H)
$$
The sum $\lambda E + H$ can be equated to $R_n - G$ using @eq-energybalance.

The Bowen ratio expresses a closely related relation and is defined as
$$
B = H/\lambda E
$$
The ratio of actual over potential evapotranspiration ($E/\mathrm{PET}$) is often used, but measurements of actual evapotranspiration are scarce and model-based estimates are subject to strong assumptions.
